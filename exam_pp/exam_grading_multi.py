...
if os.environ.get("GPU_DEVICE") != None:
    gpu_device_counter = itertools.count(start=int(os.environ.get("GPU_DEVICE", 0)))
...
def process_paragraph(args):
    ...
    gpu_device_id = next(gpu_device_counter)
    if os.environ.get("GPU_DEVICE") != None:
    ...
    if numAll > 0:
        ...
        exam_grades = ExamGrades(correctAnswered=[qpc.prompt_id() for qpc, answer in answerTuples if qpc.check_answer(answer)],
                                 ...
                                 llm_options={},
                                 prompt_type=grading_prompts[0].prompt_type(),
                                 prompt_info=grading_prompts[0].prompt_info(),
